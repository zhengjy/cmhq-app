...
<script>
	...
	export default {
		...
		computed: {
			...
			imgStyle() {
				const width = this.size === 'large' ? '17px' : this.size === 'medium' ? '15px' :this.size === 'mini'?'13px':'10px'
				return {
					width,
					height: width
				}
			...
			// 文本的样式
			closeSize() {
				const size = this.size === 'large' ? 15 : this.size === 'medium' ? 13 : this.size === 'mini'?12:10
				return size
			},
			// 图标大小
			iconSize() {
				const size = this.size === 'large' ? 21 : this.size === 'medium' ? 19 :  this.size === 'mini' ? 16 : 12
				return size
			},
			// 图标颜色
			elIconColor() {
